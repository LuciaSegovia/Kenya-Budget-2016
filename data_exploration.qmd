...
hh$sex <- as.character(hh$sex)

#create population pyramid
hh %>% group_by(age_y, sex) %>% 
...
#  scale_y_continuous(labels = abs, limits = max(hh$population) * c(-1,1)) +
  coord_flip()
...
# Checking recoded 
length(hh$school_level[hh$school_level>90 & !is.na(hh$school_level)])
#hh$school_level[hh$school_level>90 & !is.na(hh$school_level)] <- NA

haven::print_labels(hh$school_level)
...
N_sac <- length(hh$id_unique[hh$age_y>6 & hh$age_y <14])
...
N_sac_feed <- length(hh$id_unique[hh$age_y>6 & hh$age_y <14 & !is.na(hh$school_feed) &  hh$school_feed == 1])
...
hh$total_months <- ifelse(!is.na(hh$age_m), hh$age_m +(hh$age_y*12), hh$age_y*12)
...
test$id_unique <- hh$id_unique
names(test)

test %>% relocate(id_unique, .before = "clenhei") %>% 
  filter(!is.na(flen)) %>% 
  left_join(., hh %>% select(age_m, age_y, age_group, height_cm, weight_kg, hhid_unique, id_unique)) %>% 
...
test %>% relocate(id_unique, .before = "clenhei") %>% 
  filter(!is.na(flen)) %>% 
  left_join(., hh %>% select(age_m, age_y, age_group, height_cm, weight_kg, hhid_unique, id_unique)) %>% filter(flen == "1")
